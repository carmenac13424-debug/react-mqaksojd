...
function HistoriaQuick({ ant, setAnt, toast }) {
  ...
  function onInput(v) {
    ...
    setSugs([...new Set([
      ...Object.entries(AC_MAP)
        .filter(([k]) => k.startsWith(lo) || k.includes(lo))
        .map(([,val]) => val),
      ...FAV_RD
        .filter(f => f.toLowerCase().includes(lo))
        .map(f => AC_MAP[f.toLowerCase()] || f),
    ])].slice(0, 4));
  }
  ...
  return (
    <div>
        ...
        {TIPOS_ANT.map(t => (
          <button key={t.id} onClick={() => setTipo(t.id)}
            ...
            <span style={{ fontSize:8, fontWeight:700,
              color:tipo===t.id?C.teal:C.dgray, fontFamily:"Sora,sans-serif" }}>
              {t.l}
            </span>
          </button>
        ))}
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginBottom:7 }}>
          {FAV_RD.map(f => (
            <button key={f} onClick={() => add(AC_MAP[f.toLowerCase()] || f)}
              style={{ padding:"3px 9px", borderRadius:16,
                border:"1px solid "+C.border, background:C.gray,
        ...
        <div style={{ display:"flex", gap:6 }}>
          <input value={txt} onChange={e => onInput(e.target.value)}
            ...
            style={{ flex:1, padding:"9px 11px", borderRadius:9,
              ...
              background:C.off, outline:"none" }}/>
          <button onClick={() => { if (txt.trim()) add(sugs[0] || txt.trim()); }}
            style={{ padding:"9px 13px", borderRadius:9, border:"none",
              background:C.teal, cursor:"pointer", color:C.white,
              fontFamily:"Sora,sans-serif", fontWeight:700 }}>✓</button>
        </div>
        {sugs.length > 0 && (
          <div style={{ position:"absolute", top:"100%", left:0, right:0,
            background:C.white, border:"1px solid "+C.teal, borderRadius:9,
            overflow:"hidden", zIndex:10, marginTop:2,
      ...
      {total > 0 && (
        <p style={{ fontSize:10, color:C.teal, margin:"6px 0 0", fontWeight:600 }}>
  ...
  return (
    <div>
      <p style={{ fontSize:10, color:C.dgray, margin:"0 0 7px", lineHeight:1.5 }}>
        Anote los medicamentos mencionados. Los cambios van en el Plan.
      </p>
      {pend && (
        <div style={{ background:C.lteal, borderRadius:9, padding:"9px 11px",
          marginBottom:8, border:"1px solid "+C.teal+"44" }}>
          <p style={{ fontSize:11, fontWeight:700, color:C.teal, margin:"0 0 6px" }}>
            🤖 IA captó: "{pend}"
          </p>
          <div style={{ display:"flex", gap:6 }}>
            <button onClick={() => setPend(null)}
              style={{ flex:1, padding:"7px", borderRadius:8,
                ...
                cursor:"pointer", fontFamily:"Sora,sans-serif",
                fontSize:11, color:C.dgray }}>✕</button>
            <button onClick={() => {
              setMeds([...meds, { id:"m"+Date.now(), n:pend, d:"", f:"audio", s:"dijo", al:[] }]);
              setPend(null); toast("✅ " + pend);
            }} style={{ flex:2, padding:"7px", borderRadius:8, border:"none",
              background:C.teal, cursor:"pointer", fontFamily:"Sora,sans-serif",
              fontSize:12, fontWeight:800, color:C.white }}>✅ Confirmar</button>
          </div>
        </div>
      )}
      <div style={{ display:"flex", gap:7 }}>
        <input value={txt} onChange={e => setTxt(e.target.value)}
          onKeyDown={e => {
            if (e.key==="Enter" && txt.trim()) { setPend(txt.trim()); setTxt(""); }
          }}
          ...
          style={{ flex:1, padding:"8px 10px", borderRadius:9,
            ...
            background:C.off, outline:"none" }}/>
        <button onClick={() => {
          if (txt.trim()) { setPend(txt.trim()); setTxt(""); }
        }} style={{ padding:"8px 12px", borderRadius:9, border:"none",
          background:C.teal, cursor:"pointer", color:C.white,
          fontFamily:"Sora,sans-serif", fontWeight:700 }}>✓</button>
      </div>
      <p style={{ fontSize:10, color:C.dgray, margin:"6px 0 0" }}>
        {meds.length} medicamento{meds.length!==1?"s":""} registrado{meds.length!==1?"s":""}
      </p>
    </div>
  );
}
...
export default function RecordingScreen({
  ...
  return (
    <div style={{ fontFamily:"Sora,sans-serif", maxWidth:390, margin:"0 auto",
      ...
      {/* Header — solo nombre, estado, tiempo */}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          {offline && (
            <span style={{ fontSize:9, fontWeight:700, color:C.amber,
              ...
              padding:"2px 7px", borderRadius:8 }}>📵</span>
          )}
            ...
            {paused ? (
              ...
              <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                ...
                <span style={{ fontSize:11, fontWeight:600,
                  color:"rgba(255,255,255,0.6)" }}>IA activa</span>
              </div>
            )}
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column",
        alignItems:"center", justifyContent:"center", padding:"0 28px", gap:22 }}>

        {/* Waveform */}
        <div style={{ display:"flex", alignItems:"center", gap:3,
          height:52, opacity:paused?0.2:1, transition:"opacity 0.4s" }}>
          {Array.from({ length:26 }).map((_,i) => (
            <div key={i} style={{ width:3, borderRadius:3,
              background:"rgba(11,140,128,0.7)",
          ...
          {paused ? (
            <>
              <p style={{ fontWeight:700, fontSize:17,
                color:"rgba(255,255,255,0.55)", margin:"0 0 5px" }}>
                Consulta pausada
              </p>
            ...
            <>
              <p style={{ fontWeight:700, fontSize:17, color:C.white, margin:"0 0 5px" }}>
                IA activa
              </p>
              <p style={{ fontSize:12, color:"rgba(255,255,255,0.3)", margin:0, lineHeight:1.6 }}>
                Atienda a su paciente normalmente
              </p>
            </>
          )}
        </div>

        {/* Alerta clínica — discreta, solo si no pausado */}
        {!paused && (
          <div style={{ width:"100%",
      ...
      {/* Botones inferiores — Pausa + Terminar + ≡ */}
      <div style={{ padding:"0 20px 22px", flexShrink:0,
        ...
        <button onClick={() => setPanelOpen(!panelOpen)}
          style={{ width:44, height:44, borderRadius:12,
            ...
            cursor:"pointer",
            color:panelOpen?C.teal:"rgba(255,255,255,0.45)",
      ...
      {panelOpen && (
        <>
          ...
          <div style={{ position:"fixed", bottom:0, left:0, right:0,
            zIndex:140, display:"flex", justifyContent:"center" }}>
            <div style={{ width:"100%", maxWidth:390, background:C.white,
              ...
              <div style={{ padding:"8px 14px 0", flexShrink:0 }}>
                <div style={{ width:32, height:3, borderRadius:2,
                  background:C.mgray, margin:"0 auto 8px" }}/>
                  ...
                  {TABS.map(t => (
                    <button key={t.id} onClick={() => setTab(t.id)}
                      ...
                      <span style={{ fontSize:8, fontWeight:700,
                        ...
                        fontFamily:"Sora,sans-serif" }}>{t.l}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ flex:1, overflowY:"auto", padding:"10px 14px 24px" }}>
                ...
                {tab === "nota" && (
                  <div>
                    <p style={{ fontSize:11, color:C.dgray,
                      margin:"0 0 9px", lineHeight:1.5 }}>
                      El paciente no escucha esto.{" "}
                      <strong>Requieren aprobación.</strong>
                    </p>
                    {notas.length > 0 && (
                      <div style={{ marginBottom:8 }}>
                        {notas.map(n => (
                          <div key={n.id} style={{ display:"flex", gap:7,
                            padding:"5px 8px",
                    ...
                    <div style={{ display:"flex", flexDirection:"column",
                      ...
                      {NP_SUGS.map((s,i) => (
                        <button key={i} onClick={() => {
                          setNotas([...notas, {
                            id:"np"+Date.now(), t:s, f:"sugerencia", ok:false
                          }]);
                          toast("📝 Añadida");
                        }} style={{ textAlign:"left", padding:"7px 10px",
                          borderRadius:8, background:C.gray,
                          border:"1px solid "+C.border, cursor:"pointer",
                          fontFamily:"Sora,sans-serif", fontSize:11, color:C.text }}>
                          + {s}
                        </button>
                      ))}
                    </div>
