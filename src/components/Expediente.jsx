...
function Badge({ texto, color, bg }) {
  return (
    <span style={{ fontSize:9, fontWeight:700, color, background:bg,
      padding:"2px 8px", borderRadius:8, flexShrink:0 }}>{texto}</span>
  );
}

function SecHeader({ title, icon, color, count }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:8 }}>
      <span style={{ fontSize:15 }}>{icon}</span>
      <span style={{ fontSize:11, fontWeight:800, color,
        textTransform:"uppercase", letterSpacing:0.7 }}>{title}</span>
      {count !== undefined && (
        <span style={{ fontSize:9, fontWeight:700, color:C.dgray,
          background:C.gray, padding:"1px 7px", borderRadius:8 }}>{count}</span>
      )}
    </div>
  );
}
...
export default function Expediente({ onClose }) {
  ...
  const resImagenes  = exp.resultados.filter(r => r.tipo !== "laboratorio");
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:C.off,
      zIndex:400, fontFamily:"Sora,sans-serif",
      display:"flex", flexDirection:"column", maxWidth:390, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:3 }}>
          ...
          <button onClick={onClose}
            style={{ padding:"5px 13px", borderRadius:18,
              ...
              color:"rgba(255,255,255,0.6)" }}>← Cerrar</button>
        </div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"12px 14px", paddingBottom:20 }}>

        {/* ── PROBLEMAS ACTIVOS ── */}
        {tab === "problemas" && (
          <div>
            <SecHeader title="Problemas activos" icon="🩺" color={C.navy}
              count={exp.problemas.filter(p=>p.activo).length}/>
            {exp.problemas.filter(p=>p.activo).map(p => (
              <div key={p.id} style={{ background:C.white, borderRadius:11,
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <span style={{ fontSize:9, color:C.dgray }}>📅 {p.fecha}</span>
                  <span style={{ fontSize:9, color:C.dgray }}>· {p.fuente}</span>
                </div>
              </div>
            ))}
            ...
            {exp.alergias.length > 0 && (
              <div style={{ marginTop:14 }}>
                <SecHeader title="Alergias" icon="⚠️" color={C.red}
                  count={exp.alergias.length}/>
                {exp.alergias.map(a => (
        ...
        {tab === "meds" && (
          <div>
            <SecHeader title="Medicamentos actuales" icon="💊" color={C.teal}
              count={medActivos.length}/>
            {medActivos.map(m => (
              <div key={m.id} style={{ background:C.white, borderRadius:11,
                ...
                <div style={{ display:"flex", justifyContent:"space-between",
                  ...
                  <Badge texto="Activo" color={C.green} bg={C.lgreen}/>
                </div>
                {m.dosis && (
                  <div style={{ fontSize:11, color:C.dgray, marginBottom:3 }}>{m.dosis}</div>
                )}
                <div style={{ display:"flex", gap:8 }}>
                  <span style={{ fontSize:9, color:C.dgray }}>📅 {m.fecha}</span>
                  <span style={{ fontSize:9, color:C.dgray }}>· {m.fuente}</span>
                </div>
              </div>
            ))}

            <div style={{ marginTop:14 }}>
              <SecHeader title="Historial de cambios" icon="📋" color={C.dgray}/>
              {medHistorial.map((m,i) => {
                const ac = ACCION_COLOR[m.accion] || ACCION_COLOR["iniciado"];
                return (
                  <div key={m.id+i} style={{ background:C.white, borderRadius:10,
                    border:"1px solid "+C.border, padding:"8px 12px", marginBottom:5 }}>
                    <div style={{ display:"flex", alignItems:"center",
                      justifyContent:"space-between", marginBottom:3 }}>
                      <span style={{ fontSize:12, fontWeight:600, color:C.text,
                        flex:1, paddingRight:8 }}>{m.nombre}</span>
                      <Badge texto={ac.label} color={ac.color} bg={ac.bg}/>
                    </div>
                    {m.dosis && (
            ...
            {exp.resultados.length === 0 ? (
              <div style={{ textAlign:"center", padding:"30px 20px" }}>
                ...
                <div style={{ fontSize:11, color:C.dgray, lineHeight:1.6 }}>
                  Los resultados de Amadita, Referencia, Cedimat<br/>
                  y otros apareceran aquí visita por visita.
                </div>
              </div>
            ) : (
              <>
                <SecHeader title="Laboratorios" icon="🧪" color={C.blue}
                  count={resLabs.length}/>
                {resLabs.map(r => (
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between",
                      ...
                      {r.valor && (
                        <span style={{ fontFamily:"JetBrains Mono,monospace",
                          ...
                          {r.valor}{r.unidad && <span style={{ fontSize:10, color:C.dgray, marginLeft:2 }}>{r.unidad}</span>}
                        </span>
                      )}
                    </div>
                    <div style={{ display:"flex", gap:8 }}>
                      {r.lab && <span style={{ fontSize:9, fontWeight:700,
                        color:C.teal, background:C.lteal,
                        padding:"1px 6px", borderRadius:6 }}>{r.lab}</span>}
                      <span style={{ fontSize:9, color:C.dgray }}>📅 {r.fecha}</span>
                    </div>
                  </div>
                ))}

                {resImagenes.length > 0 && (
                  <div style={{ marginTop:14 }}>
                    <SecHeader title="Imágenes / EKG / Otros" icon="🫁" color="#374151"
                      count={resImagenes.length}/>
                    {resImagenes.map(r => (
                      <div key={r.id} style={{ background:C.white, borderRadius:11,
        ...
        {tab === "visitas" && (
          <div>
            <SecHeader title="Historial de visitas" icon="📋" color={C.navy}
              count={exp.visitas.length}/>
                ...
                <div key={v.id} style={{ background:C.white, borderRadius:12,
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between",
                    ...
                    <span style={{ fontSize:9, fontWeight:700,
                      color:C.green, background:C.lgreen,
                      padding:"2px 8px", borderRadius:8 }}>✅ Firmada</span>
                  </div>
                  ...
                  <div style={{ display:"flex", gap:10, marginTop:6 }}>
                    <span style={{ fontSize:9, color:C.dgray }}>
                      💊 {v.resumen.meds_count} meds
                    </span>
                    <span style={{ fontSize:9, color:C.dgray }}>
                      🧪 {v.resumen.resultados_count} resultados
                    </span>
                    <span style={{ fontFamily:"JetBrains Mono,monospace",
                      fontSize:9, color:C.mgray }}>
                      {v.firmaId}
                    </span>
                  </div>
                </div>
        ...
        {/* Zona de reset — solo para testing */}
        <div style={{ borderTop:"1px solid "+C.border,
          paddingTop:14, marginTop:14, textAlign:"center" }}>
          {!confirm ? (
            <button onClick={() => setConfirm(true)}
              style={{ background:"transparent", border:"none",
                cursor:"pointer", fontSize:10,
                color:C.mgray, fontFamily:"Sora,sans-serif" }}>
              Borrar expediente (testing)
            </button>
          ) : (
            <div>
              ...
              <div style={{ display:"flex", gap:8, justifyContent:"center" }}>
                ...
                <button onClick={() => { clearExpediente(); setExp(defaultExp()); setConfirm(false); }}
                  style={{ padding:"5px 14px", borderRadius:9,
                    border:"none", background:C.red,
                    cursor:"pointer", fontFamily:"Sora,sans-serif",
                    fontSize:11, fontWeight:700, color:C.white }}>Borrar todo</button>
              </div>
            </div>
          )}
        </div>
